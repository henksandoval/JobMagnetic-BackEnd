...
namespace JobMagnet.Infrastructure.CvParsers;

internal partial class GeminiCvParser(IOptions<GeminiSettings> options, ILogger<GeminiCvParser> logger) : ICvParser
{
    private readonly GeminiSettings _settings = options.Value;

    public async Task<Maybe<ProfileEntity>> ParseAsync(IFormFile cvFile)
    {
        if (cvFile.Length == 0)
        {
            logger.LogError("CV file is null or empty.");
    ...
    private async Task<Maybe<string>> ReadAndValidateCvFileAsync(IFormFile cvFile)
    {
        string cvContent;
